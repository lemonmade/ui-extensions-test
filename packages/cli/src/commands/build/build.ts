...
// the package.
const brotliSize =
  'default' in brotliSizeModule
    ? (brotliSizeModule as unknown as {default: typeof brotliSizeModule})
        .default
    : brotliSizeModule;
...
export async function build({ui}: {ui: Ui}) {
  ...
  const builds = await Promise.all(
    app.extensions.map(async (extension) => {
      const bundle = await rollup(
        await createRollupConfiguration(extension, {mode: 'production'}),
      );
      ...
      return {
        extension,
        size: code.length,
        minifiedSize: await brotliSize(code),
      };
    }),
  );
  ...
  if (app.extensions.length === 1) {
    ui.TextBlock(`Built extension ${ui.Code(app.extensions[0]!.name)}!`);
  } else {
    ui.TextBlock(`Built ${app.extensions.length} extensions:`);
    ui.List((List) => {
      for (const {extension, size, minifiedSize} of builds) {
        const sizeContent = ui.Text(
          `(${prettyBytes(size)}, ${prettyBytes(minifiedSize)} minifed)`,
          {
            style: (content, style) => {
